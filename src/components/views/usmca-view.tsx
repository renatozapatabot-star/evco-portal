      ...
      <div style={{ display: 'grid', gridTemplateColumns: cert ? '1fr 1fr' : '600px', gap: 20 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, boxShadow: T.shadow, padding: 20 }}>
            <div style={{ color: T.textMuted, fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>Proveedor / Exportador</div>
            <select onChange={handleSupplierSelect} style={{ width: '100%', height: 38, border: `1px solid ${T.border}`, borderRadius: 8, padding: '0 10px', fontSize: 13, color: T.text, background: T.bg, outline: 'none', fontFamily: 'inherit', marginBottom: 10 }}>
          ...
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, boxShadow: T.shadow, padding: 20 }}>
            <div style={{ color: T.textMuted, fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>Goods / Mercancías</div>
            <div style={{ marginBottom: 10 }}>
              <label style={{ display: 'block', color: T.textSub, fontSize: 11, fontWeight: 600, marginBottom: 4 }}>Description of Goods *</label>
              <textarea value={form.goods_description} onChange={e => set('goods_description', e.target.value)} placeholder="Plastic resin pellets, polypropylene..." rows={3} style={{ width: '100%', border: `1px solid ${T.border}`, borderRadius: 7, padding: '8px 10px', fontSize: 12, color: T.text, background: T.bg, outline: 'none', fontFamily: 'inherit', resize: 'vertical', boxSizing: 'border-box' }} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
                <label style={{ display: 'block', color: T.textSub, fontSize: 11, fontWeight: 600, marginBottom: 4 }}>HS Code *</label>
                <input value={form.hs_code} onChange={e => set('hs_code', e.target.value)} placeholder="3902.10" style={{ width: '100%', height: 36, border: `1px solid ${T.border}`, borderRadius: 7, padding: '0 10px', fontSize: 12, color: T.text, background: T.bg, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ display: 'block', color: T.textSub, fontSize: 11, fontWeight: 600, marginBottom: 4 }}>Origin Criterion</label>
                <select value={form.origin_criterion} onChange={e => set('origin_criterion', e.target.value)} style={{ width: '100%', height: 36, border: `1px solid ${T.border}`, borderRadius: 7, padding: '0 8px', fontSize: 12, color: T.text, background: T.bg, outline: 'none', fontFamily: 'inherit' }}>
          ...
          <button onClick={generateCertificate} disabled={generating || !form.goods_description || !form.hs_code} style={{ height: 44, background: generating || !form.goods_description || !form.hs_code ? '#CBD5E1' : T.navy, border: 'none', borderRadius: 10, color: 'rgba(255,255,255,0.045)', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>{generating ? '⏳ Validating...' : '📜 Generate USMCA Certificate'}</button>
        </div>
        {cert && (
          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, boxShadow: T.shadow, padding: 20, borderTop: `3px solid ${T.gold}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              <button onClick={downloadCert} style={{ background: T.navy, border: 'none', borderRadius: 7, padding: '7px 14px', color: 'rgba(255,255,255,0.045)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>⬇️ Download</button>
            </div>
            ...
            {cert.analysis && <div style={{ marginTop: 14, padding: 12, background: T.surfaceAlt, border: `1px solid ${T.border}`, borderRadius: 8 }}><div style={{ color: T.textMuted, fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 6 }}>Origin Analysis</div><div style={{ color: T.textSub, fontSize: 12, lineHeight: 1.6 }}>{cert.analysis}</div></div>}
            <div style={{ marginTop: 16, paddingTop: 12, borderTop: `1px solid ${T.border}`, textAlign: 'right' }}><div style={{ color: T.text, fontSize: 12, fontWeight: 700 }}>{form.certifier_name}</div><div style={{ color: T.textMuted, fontSize: 10 }}>Director General · Patente 3596</div></div>
          </div>
        )}
      </div>
    </div>
