...
/**
 * V1 Polish Pack · Block 1 — tráfico detail server actions.
 *
 * Two actions:
...
export async function updateTraficoStatus(traficoId: string, newStatus: string): Promise<ActionResult> {
  ...
  if (!existing) return { ok: false, error: 'Tráfico no encontrado' }
  ...
  revalidatePath(`/traficos/${traficoId}`)
  return { ok: true, error: null }
}
...
export async function addTraficoNote(
  ...
  // Verify caller can see this tráfico before we log anything.
  ...
  if (!scopeRow) return { ok: false, error: 'Tráfico no encontrado' }
  ...
  for (const m of cleanMentions) {
    ...
    await createNotification({
      ...
      severity: 'info',
      actionUrl: `/traficos/${encodeURIComponent(traficoId)}`,
      traficoId,
