    ...
    <div style={{ position: 'relative' }}>
      <input
        ...
        style={{
          ...
          border: `1.5px solid ${error ? 'var(--portal-status-red-fg)' : 'rgba(192,197,206,0.3)'}`,
        ...
        onFocus={e => { e.currentTarget.style.borderColor = error ? 'var(--portal-status-red-fg)' : 'rgba(192,197,206,0.5)'; e.currentTarget.style.boxShadow = `0 0 12px ${error ? 'rgba(239,68,68,0.2)' : 'rgba(192,197,206,0.2)'}` }}
        onBlur={e => { e.currentTarget.style.borderColor = error ? 'var(--portal-status-red-fg)' : 'rgba(192,197,206,0.3)'; e.currentTarget.style.boxShadow = 'none' }}
      />
      {error && (
        <div style={{ fontSize: 'var(--aguila-fs-meta)', color: 'var(--portal-status-red-fg)', marginTop: 4 }}>{error}</div>
      )}
    </div>
  )
}
