  ...
  if (!proposal) {
    return (
      <div style={{
        ...
        <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--portal-fg-5)', flexShrink: 0 }} />
        <span style={{ fontSize: 'var(--aguila-fs-body)', color: 'var(--portal-fg-5)' }}>
    ...
    <>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 'var(--aguila-fs-meta)', color: 'var(--portal-fg-5)', flexShrink: 0 }}>
        ...
        {proposal.reasoning_bullets && proposal.reasoning_bullets.length > 0 && (
          <div style={{ marginBottom: 16, paddingLeft: 12, borderLeft: '2px solid rgba(192,197,206,0.2)' }}>
            {proposal.reasoning_bullets.map((b: { text: string }, i: number) => (
              <div key={i} style={{ fontSize: 'var(--aguila-fs-body)', color: 'var(--portal-fg-4)', lineHeight: 1.6, marginBottom: 4 }}>
                {b.text}
              </div>
            ))}
          </div>
        ...
        {proposal.alternatives && proposal.alternatives.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 'var(--aguila-fs-label)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--portal-fg-5)', marginBottom: 6 }}>
              Alternativas consideradas
            </div>
            {proposal.alternatives.map((alt: { action: string; label_es: string; confidence: number }, i: number) => (
              <div key={i} style={{ fontSize: 'var(--aguila-fs-compact)', color: 'var(--portal-fg-5)', marginBottom: 2 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{
              ...
              background: 'transparent', color: 'var(--portal-fg-4)', fontSize: 'var(--aguila-fs-body)', fontWeight: 600,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button onClick={() => setShowRejectModal(false)} style={{
              ...
              background: 'none', border: 'none', color: 'var(--portal-fg-5)', fontSize: 'var(--aguila-fs-body)',
              cursor: 'pointer',
            }}>
              Cancelar
            </button>
