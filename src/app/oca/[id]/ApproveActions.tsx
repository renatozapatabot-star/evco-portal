...
export function ApproveActions({ opinionId, canApprove, status }: Props) {
  ...
  async function runApproval() {
    ...
    setPhase('submitting')
    try {
      const res = await fetch(`/api/oca/${opinionId}/approve`, { method: 'POST' })
      const json = await res.json() as { data: unknown; error: { message: string } | null }
      if (!res.ok || !json.data) {
        setErr(json.error?.message ?? 'No se pudo aprobar la opinión')
        setPhase('idle')
        return
      }
      router.refresh()
  ...
  const pdfHref = `/api/oca/${opinionId}/pdf`
