    ...
    >
      ...
      >
        ...
        <div className="portal-modules-grid">
          ...
          {/* 2. Tile #2 — Pedimentos for clients, Contabilidad for broker/operator.
               Per audit Cluster G (2026-05-05) the client-side Contabilidad
               surface is fully hidden — /mi-cuenta + /contabilidad/* still
               302→/inicio for client roles, and the prior tile linked to a
               non-functional surface ("218 ABIERTAS" with no destination).
               NEXT_PUBLIC_MI_CUENTA_ENABLED retired here; the broker/operator
               path keeps the working /contabilidad/inicio (Anabel's cockpit). */}
          {isClient ? (
            <PortalModuleCard
            ...
            <PortalModuleCard
              ...
              title="Contabilidad"
              desc="CxC · CxP · cierre del mes."
              badge={
                navCounts.contabilidad?.count
                  ? { tone: 'info', label: `${navCounts.contabilidad.count} ABIERTAS` }
                  : undefined
              }
              viz={<VizPedimentoLedger />}
              metric={fmt(navCounts.contabilidad?.count ?? null, '0')}
              metricLabel="CxC ABIERTAS"
              href={navHref('contabilidad', '/contabilidad/inicio')}
              accent
              ariaLabel="Ir a Contabilidad"
            />
          )}
