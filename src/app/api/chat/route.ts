...
async function getContextData(query: string, companyId: string, clientClave: string) {
  ...
  if (q.includes('proveedor') || q.includes('supplier') || q.includes('top')) {
    ...
    const byP: Record<string, number> = {}; (data || []).forEach((f: { proveedor?: string | null; valor_usd?: number | null }) => { if (f.proveedor) byP[f.proveedor] = (byP[f.proveedor] || 0) + (f.valor_usd || 0) })
