...
export async function GET(request: NextRequest) {
  ...
  const [trafRes, entRes] = await Promise.all([
    supabase.from('traficos').select('trafico, estatus, transportista_extranjero, transportista_mexicano, fecha_llegada, peso_bruto').eq('company_id', companyId).not('transportista_extranjero', 'is', null).limit(5000),
