...
const T = { surface: 'var(--card-bg)', surfaceAlt: '#F5F3EF', border: '#E8E6E0', text: '#1A1A1A', textSub: '#6B6B6B', textMuted: '#999999', navy: '#1A1A1A', gold: '#BA7517', shadow: '0 8px 32px rgba(0,0,0,0.12)' }
...
export function AIChat() {
  ...
  const ZAvatar = () => (
    <div style={{ width: 28, height: 28, background: 'var(--amber-600, #BA7517)', borderRadius: 6, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round"><path d="M12 2L13.5 8.5L20 10L13.5 11.5L12 18L10.5 11.5L4 10L10.5 8.5L12 2Z"/></svg>
    </div>
  )
        ...
        <div style={{ position: 'fixed', bottom: 80, right: 24, width: 380, height: 520, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, boxShadow: T.shadow, zIndex: 199, display: 'flex', flexDirection: 'column', fontFamily: 'var(--font-geist-sans)', overflow: 'hidden' }}>
          <div style={{ background: 'var(--amber-600, #BA7517)', padding: '14px 16px', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div><div style={{ color: '#fff', fontSize: 14, fontWeight: 700 }}>CRUZ Intelligence</div><div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 11 }}>{getClientNameCookie().split(' ')[0]} Plastics &middot; Patente {PATENTE}</div></div>
            </div>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {messages.map((m, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: m.role === 'user' ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: 8 }}>
                ...
                <div style={{ maxWidth: '75%', padding: '9px 12px', borderRadius: 12, borderBottomLeftRadius: m.role === 'assistant' ? 4 : 12, borderBottomRightRadius: m.role === 'user' ? 4 : 12, background: m.role === 'user' ? 'var(--amber-600, #BA7517)' : T.surfaceAlt, color: m.role === 'user' ? '#fff' : T.text, fontSize: 13, lineHeight: 1.5 }}>
          ...
          <div style={{ padding: '10px 12px', borderTop: `1px solid ${T.border}`, display: 'flex', gap: 8, flexShrink: 0 }}>
            ...
            <button onClick={() => sendMessage()} disabled={!input.trim() || loading} style={{ width: 38, height: 38, background: input.trim() && !loading ? 'var(--amber-600, #BA7517)' : T.border, border: 'none', borderRadius: 8, color: '#fff', fontSize: 16, cursor: input.trim() && !loading ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>&rarr;</button>
          </div>
        </div>
      )}
    </>
