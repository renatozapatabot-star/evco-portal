...
const ACTION_LABELS: Record<string, { icon: string; label: string }> = {
  ...
  cruz_ai_query: { icon: '🤖', label: 'preguntó a Asistente PORTAL' },
...
export function TeamActivityFeed() {
  ...
  return (
    <IfThenCard
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 300, overflowY: 'auto' }}>
            {feed.map((item, i) => (
              <div key={item.id} style={{
                ...
                <span className="font-mono" style={{ fontSize: 'var(--aguila-fs-label)', color: 'var(--portal-fg-5)', flexShrink: 0 }}>
                  {item.time}
                </span>
              </div>
            ))}
      ...
      footer={
        <span style={{ color: isLive ? 'var(--portal-status-green-fg)' : 'var(--portal-fg-5)' }}>
