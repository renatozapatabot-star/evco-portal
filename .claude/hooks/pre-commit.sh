...
[ "$(grep -rn "'9254'" src/app/ --include="*.tsx" 2>/dev/null | grep -v config | wc -l | tr -d ' ')" = "0" ] && echo "No hardcoded IDs" || { echo "Hardcoded '9254' found"; exit 1; }
[ "$(grep -rn "alert(" src/ --include="*.tsx" 2>/dev/null | wc -l | tr -d ' ')" = "0" ] && echo "No alert()" || { echo "alert() found"; exit 1; }
