...
export function AIChat() {
  ...
  useEffect(() => { if (open) { setTimeout(() => inputRef.current?.focus(), 100); if (messages.length === 0) setMessages([{ role: 'assistant', content: `Hola, soy ZAPATA. Puedo consultar datos ${getClientNameCookie().split(' ')[0]} en tiempo real. ¿Qué necesitas?`, ts: new Date().toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' }) }]) } }, [open])
  ...
  const ZAvatar = () => (
    <div style={{ width: 28, height: 28, background: 'var(--amber-600, #BA7517)', borderRadius: 6, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.045)" strokeWidth="2" strokeLinecap="round"><path d="M12 2L13.5 8.5L20 10L13.5 11.5L12 18L10.5 11.5L4 10L10.5 8.5L12 2Z"/></svg>
    </div>
  )
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(o => !o)}
        title="ZAPATA AI Intelligence"
        aria-label="Abrir ZAPATA AI"
        style={{
          ...
          transition: 'all 0.2s ease', color: 'rgba(255,255,255,0.045)',
        }}
      ...
      >
        {open ? (
          <span style={{ fontSize: 'var(--aguila-fs-kpi-small)', fontWeight: 600 }}>&times;</span>
        ) : (
        ...
        <div style={{ position: 'fixed', bottom: 80, right: 24, width: 380, height: 520, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, boxShadow: T.shadow, zIndex: 199, display: 'flex', flexDirection: 'column', fontFamily: 'var(--font-geist-sans)', overflow: 'hidden' }}>
          <div style={{ background: 'var(--amber-600, #BA7517)', padding: '14px 16px', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div><div style={{ color: 'rgba(255,255,255,0.045)', fontSize: 'var(--aguila-fs-section)', fontWeight: 700 }}>ZAPATA AI Intelligence</div><div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 'var(--aguila-fs-meta)' }}>{getClientNameCookie().split(' ')[0]} Plastics &middot; Patente {PATENTE}</div></div>
            </div>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {messages.map((m, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: m.role === 'user' ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: 8 }}>
                ...
                <div style={{ maxWidth: '75%', padding: '9px 12px', borderRadius: 12, borderBottomLeftRadius: m.role === 'assistant' ? 4 : 12, borderBottomRightRadius: m.role === 'user' ? 4 : 12, background: m.role === 'user' ? 'var(--amber-600, #BA7517)' : T.surfaceAlt, color: m.role === 'user' ? 'rgba(255,255,255,0.045)' : T.text, fontSize: 'var(--aguila-fs-body)', lineHeight: 1.5 }}>
                  {m.content}<div style={{ color: m.role === 'user' ? 'rgba(255,255,255,0.4)' : T.textMuted, fontSize: 9, marginTop: 4, textAlign: 'right' }}>{m.ts}</div>
                </div>
              </div>
            ))}
            {loading && <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}><ZAvatar /><div style={{ background: T.surfaceAlt, borderRadius: 12, borderBottomLeftRadius: 4, padding: '10px 14px', color: T.textMuted, fontSize: 'var(--aguila-fs-compact)' }}>Consultando datos...</div></div>}
            {messages.length === 1 && <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 4 }}>{SUGGESTIONS.map((s, i) => <button key={i} onClick={() => sendMessage(s)} style={{ background: 'none', border: `1px solid ${T.border}`, borderRadius: 8, padding: '7px 10px', textAlign: 'left', cursor: 'pointer', color: T.textSub, fontSize: 'var(--aguila-fs-meta)', fontFamily: 'inherit' }}>{s}</button>)}</div>}
            <div ref={bottomRef} />
          </div>
          <div style={{ padding: '10px 12px', borderTop: `1px solid ${T.border}`, display: 'flex', gap: 8, flexShrink: 0 }}>
            <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage()} placeholder="Pregunta sobre tus operaciones..." style={{ flex: 1, height: 38, border: `1px solid ${T.border}`, borderRadius: 8, padding: '0 12px', fontSize: 'var(--aguila-fs-body)', color: T.text, background: T.surfaceAlt, outline: 'none', fontFamily: 'inherit' }} />
            <button onClick={() => sendMessage()} disabled={!input.trim() || loading} style={{ width: 38, height: 38, background: input.trim() && !loading ? 'var(--amber-600, #BA7517)' : T.border, border: 'none', borderRadius: 8, color: 'rgba(255,255,255,0.045)', fontSize: 'var(--aguila-fs-body-lg)', cursor: input.trim() && !loading ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>&rarr;</button>
          </div>
        </div>
      )}
    </>
