/**
 * /operador/trafico — CLAUDIA's cockpit.
 */
...
export default async function TraficoCockpitPage() {
  const jar = await cookies()
  const role = jar.get('user_role')?.value
  const name = jar.get('operator_name')?.value || 'Claudia'
  if (!role || !['admin', 'broker', 'operator'].includes(role)) redirect('/')
  return (
    <Suspense fallback={<CockpitSkeleton />}>
      <TraficoCockpitContent name={name} />
    </Suspense>
  )
}

async function TraficoCockpitContent({ name }: { name: string }) {
          ...
          {data.recentTraficos.slice(0, 5).map((t) => {
            ...
            return (
              <div key={t.id}>
                ...
                <ChainView nodes={chain} compact ariaLabel={`Cadena de ${t.trafico}`} />
              </div>
            )
          })}
        </div>
