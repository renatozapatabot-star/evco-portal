...
ROLE_COOKIE_COUNT=$(set +eo pipefail;{ grep -rnE "cookieStore\.get\(.user_role.\)|cookies\(\)\.get\(.user_role.\)|req\.cookies\.get\(.user_role.\)|req\.cookies\.get\(.company_id.\)" src --include="*.ts" --include="*.tsx" 2>/dev/null || true; } | grep -v "api/debug/whoami" | grep -v "lib/route-guards.ts" | grep -v "__tests__/" | grep -v "lib/api/tenant-scope.ts" | wc -l | tr -d ' ')
...
AGUILA_FORM_BASELINE=${AGUILA_FORM_BASELINE:-70}
