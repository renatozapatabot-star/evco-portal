...
/**
 * Block 1B · Tráfico detail — server actions.
 *
 ...
 * non-internal roles cannot touch tráficos outside their company_id.
 */
...
async function scopeTrafico(traficoId: string) {
  ...
  if (!data) return { session, row: null, error: 'Tráfico no encontrado' } as const
...
export async function fireLifecycleEvent(
  ...
  if (!row) return { ok: false, error: error ?? 'Tráfico no encontrado' }
