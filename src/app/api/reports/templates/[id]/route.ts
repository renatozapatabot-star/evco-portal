...
export async function PATCH(
  ...
  if (existing.data.scope === 'seed') {
    return NextResponse.json(
      { data: null, error: { code: 'FORBIDDEN', message: 'Las plantillas de Portal no son editables' } },
      { status: 403 },
    )
  }
...
export async function DELETE(
  ...
  if (existing.data.scope === 'seed') {
    return NextResponse.json(
      { data: null, error: { code: 'FORBIDDEN', message: 'Las plantillas de Portal no se eliminan' } },
      { status: 403 },
    )
  }
  const userId = `${session.companyId}:${session.role}`
