...
export async function GET(req: NextRequest) {
  ...
  if (type === 'benchmark') {
    const { data } = await supabase.from('client_benchmarks')
      .select('*').eq('company_id', 'evco').limit(10)
    return NextResponse.json(envelope(data))
  }

  if (type === 'compliance') {
    const { data: predictions } = await supabase.from('compliance_predictions')
      .select('*').eq('company_id', 'evco').eq('resolved', false)
  ...
  const [risk, comp, bench, bridge] = await Promise.all([
    ...
    supabase.from('client_benchmarks').select('metric_name, client_value, industry_avg').eq('company_id', 'evco'),
