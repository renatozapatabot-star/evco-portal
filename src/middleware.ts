...
const PUBLIC_PATHS = ['/login', '/signup', '/onboarding', '/demo', '/pitch']
...
export async function middleware(request: NextRequest) {
  ...
  // Demo + pitch routes are always public (no redirect if authenticated —
  // an authenticated user should still be able to share the pitch URL)
  ...
  if (role === 'operator') {
    ...
    const OPERATOR_ALLOWED = ['/', '/operador', '/embarques', '/entradas', '/pedimentos', '/expedientes', '/clasificar', '/fracciones', '/cumplimiento', '/bodega', '/aduana', '/cambiar-contrasena', '/inteligencia', '/health', '/catalogo', '/anexo-24', '/reportes', '/financiero', '/facturacion', '/documentos']
